...
contract IPFY {
    ...
    function putforSell(uint256 _id, uint256 _price) public {
        ...
        require(!IPDetails[_id].selling, "Already in");
    ...
    function buy(uint256 _id) public payable {
        require(IPDetails[_id].selling, "Not for sale");
        require(
    ...
    function lend(uint256 _id, uint256 months) public payable {
        ...
        require(IPDetails[_id].lending, "Not for sale");
        ...
        payable(IPDetails[_id].currenOwner).transfer(msg.value);
        bool found = false;
        for (uint i = 0; i < userLends[msg.sender].length; i++) {
            if (userLends[msg.sender][i] == _id) {
                found = true;
                break;
            }
        }
        if (!found) userLends[msg.sender].push(_id);
    }

    function checkLendablity(uint256 _id) public view returns (bool) {
        if (IPDetails[_id].lending || IPDetails[_id].selling) return false;
        require(
            !checkCurrentLendingStatus(_id),
            "this project is currently on lend"
        );
        return true;
    }
